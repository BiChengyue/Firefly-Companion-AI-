...
class WsServerFixture:
    ...
    def __init__(self, tmp_path, mode_switch_fn=None):
    ...
    def start(self):
        async def serve():
            async with websockets.serve(
                make_desktop_handler(self.hub, self.input_bus, mode_switch_fn=self.mode_switch_fn),
                "127.0.0.1",
                self.port,
...
def test_mode_switch_calls_companion_and_acks(tmp_path):
    """mode_switch 入站 → companion mode 切换被调用 → 回 mode_switched。"""
    calls = []

    def fake_switch(mode):
        calls.append(mode)
        return {"current": mode, "hudVisible": False, "thinkVisible": False, "proactiveCare": True}
    ...
    async def run():
        async with await _connect(fx.url()) as ws:
            ...
            msg = json.loads(await asyncio.wait_for(ws.recv(), timeout=3))
            assert msg == {"type": "mode_switched", "mode": "work"}
    asyncio.run(run())
    assert calls == ["work"]
    fx.hub.close_all()
